...
namespace EllenExplorer.Tools.Tiles {
    ...
    public class ScriptedTile : TileBase {
        ...
        public class Rule : RuleOutput {
            ...
            public Dictionary<Vector3Int, int> neighbors = new Dictionary<Vector3Int, int>();

            public Dictionary<Vector3Int, int> GetNeighbors() {
                return neighbors;
            }

            public void ApplyNeighbors(Dictionary<Vector3Int, int> neighbors) {
                neighborsMatrixPosition = neighbors.Keys.ToList();
                neighborsRuleCode = neighbors.Values.ToList();

                for (int neighborIndex = 0; neighborIndex < neighborsMatrixPosition.Count; neighborIndex++) {
                    this.neighbors.Add(neighborsMatrixPosition[neighborIndex], neighborsRuleCode[neighborIndex]);
                }
            }

            public BoundsInt GetMatrixBounds() {
                /**
        ...
        private bool PigMethod (int neighborRuleCode, TileBase tileByNeighborPosition) {
            ...
            switch (neighborRuleCode) {
                case RuleOutput.NeighborRuleCode.MustHave:
                    return tileByNeighborPosition == thisRuleTile;
                case RuleOutput.NeighborRuleCode.MustNotHave:
                    return tileByNeighborPosition != thisRuleTile;
            }
